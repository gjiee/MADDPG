...
import random
import numpy as np
from env_random import MultiUAVEnv
...
def run():
    ...
    memory = MultiAgentReplayBuffer(300_000, critic_dims, actor_dims,
                                    n_actions, n_agents, batch_size=1024)

    EVAL_INTERVAL = 50  # 每隔 10 步进行评估
    ...
    while total_steps < MAX_STEPS:
        ...
        while not any(dones):
            ...
            next_observations, rewards, dones_dict, trunc, infos = env.step(actions)
            # 处理 terminal 判断
            ...
            reward = [rewards[agent_id] for agent_id in uav_ids]
    ...
    np.save('../data/random_maddpg_scores.npy', np.array(eval_scores))
    np.save('../data/random_maddpg_steps.npy', np.array(eval_steps))
