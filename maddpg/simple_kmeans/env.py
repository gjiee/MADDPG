...
import copy
import random
from cluster_without_radius import *
...
class UAV:
    ...
    def apply_action(self, action):
        ...
        # 动作拆分
        compute_action = action[:MAX_DEVICES_KMEANS][:num_devices]
        bandwidth_action = action[MAX_DEVICES_KMEANS:2 * MAX_DEVICES_KMEANS][:num_devices]
    ...
    def get_observation(self):
        ...
        # 连接设备的信息
        devices_info = []
        for device in self.connected_devices[:MAX_DEVICES_KMEANS]:
            device_info = [
        ...
        num_padded_devices = MAX_DEVICES_KMEANS - len(self.connected_devices)
...
class MultiUAVEnv:
    def __init__(self):
        ...
        clustering = SimpleKMeansClustering(
            devices=self.devices,
            fixed_k=len(centroids),
        )
        ...
        for uav_id, uav in self.uavs.items():
            # 定义每个 UAV 的观测空间
            obs_dim = 5 + MAX_DEVICES_KMEANS * 6  # 5 是 uav_info 的长度，6 是每个设备的信息长度（位置2 + 任务4）
            ...
            # 定义每个 UAV 的动作空间
            action_dim = 2 * MAX_DEVICES_KMEANS  # 前 MAX_DEVICES 为计算资源分配，后 MAX_DEVICES 为带宽分配
    ...
    def compute_reward(self, uav, max_eu):
        ...
        动态调整权重并优化时延惩罚。
        """
        w_completion = 0.8  # 增强任务完成率的影响
        w_delay = 0.1 + 0.2 * (self.time_step / MAX_TIME_STEPS)  # 随时间增加对时延的关注
        w_energy = 0.01

        # 时延满意度
        delays = [device.total_delay / device.max_delay for device in uav.connected_devices if device.max_delay > 0]
        min_delay_satisfaction = min(delays) if delays else 1.0

        # 任务完成率
        completed_tasks = sum([1 for device in uav.connected_devices if device.is_task_completed])
        total_tasks = len(uav.connected_devices)
        completion_rate = completed_tasks / total_tasks if total_tasks > 0 else 1.0

        # 非线性时延惩罚
        nonlinear_delay_penalty = min_delay_satisfaction ** 2

        # 奖励 = 任务完成率 + 时延满意度 - 能耗惩罚
        reward = (
                w_completion * completion_rate
                - w_delay * nonlinear_delay_penalty
                - w_energy * max_eu
        )
