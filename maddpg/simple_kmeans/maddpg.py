...
class MADDPG:
    ...
    #              chkpt_dir='tmp/maddpg/', scenario='multi_uav'):
    #     self.agents = []  # 用于存储所有智能体（Agent）对象的列表
    #     chkpt_dir += scenario  # 将 scenario 添加到 chkpt_dir 以区分不同场景的模型保存路径
    #
    #     uav_ids = sorted(env.uavs.keys())  # 按照键的顺序排序
    #     for agent_idx, agent_id in enumerate(uav_ids):
    #         uav = env.uavs[agent_id]
    ...
    #
    #         # 创建 Agent 实例
    #         agent = Agent(actor_dims[agent_idx], critic_dims,
