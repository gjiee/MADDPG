...
class UAV:
    ...
    def apply_action(self, action):
        ...
        # 动作拆分
        compute_action = action[:MAX_DEVICES][:num_devices]
        bandwidth_action = action[MAX_DEVICES:2 * MAX_DEVICES][:num_devices]
    ...
    def get_observation(self):
        ...
        # 连接设备的信息
        devices_info = []
        for device in self.connected_devices[:MAX_DEVICES]:
            device_info = [
        ...
        num_padded_devices = MAX_DEVICES - len(self.connected_devices)
...
class MultiUAVEnv:
    def __init__(self):
        ...
        clustering = ISODATA_KMeans_Clustering(
        ...
        for uav_id, uav in self.uavs.items():
            # 定义每个 UAV 的观测空间
            obs_dim = 5 + MAX_DEVICES * 6  # 5 是 uav_info 的长度，6 是每个设备的信息长度（位置2 + 任务4）
            ...
            # 定义每个 UAV 的动作空间
            action_dim = 2 * MAX_DEVICES  # 前 MAX_DEVICES 为计算资源分配，后 MAX_DEVICES 为带宽分配
